...
// Pure: prefer the created review's own html_url; pending reviews can come back
// with an empty one, so fall back to the PR's Files-changed tab where the user
// finishes & submits the draft.
export function reviewUrl(
  ...
  return htmlUrl && htmlUrl.length > 0
    ? htmlUrl
    : `https://github.com/${owner}/${repo}/pull/${pull_number}/files`
}

// Pure: turn the renderer's payload into the GitHub create-review request body.
// Deliberately OMITS both `event` (→ GitHub creates a PENDING/draft review, never
// auto-submitted) and `body` (→ no summary comment; the AI assessment stays in
// the dashboard). Only the inline line comments are posted. Drops any comment
// without a usable numeric line so a single bad anchor can't 422 the whole
// review; omits `comments` entirely when none remain.
export function buildReviewRequest(
  ...
  const req: ReviewRequest = { owner, repo, pull_number }
...
export async function postReview(
  ...
  try {
    ...
    return { ok: true, url: reviewUrl(res.data?.html_url, owner, repo, pull_number) }
  } catch (err) {
