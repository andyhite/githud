...
// Picks the icon shape that best describes a notification entry. A real comment
// wins (it's the most useful signal); otherwise we fall back to the subject
// type. The icon's *color* is set by severity at the row level, not here.
export function ActivityIcon({ item }: { item: ActivityItem }) {
  ...
  if (item.latestComment) {
    ...
    switch (item.subjectType) {
      case 'PullRequest': name = 'pullRequest'; break
      case 'Issue': name = 'issue'; break
      case 'Commit': name = 'commit'; break
      case 'Release': name = 'release'; break
      default: name = 'bell'
    }
  }
  return (
    <span className="activity-icon" aria-label={item.subjectType}>
      <Octicon name={name} />
    </span>
  )
}
