...
src/
  ...
  renderer/src/            # UI is Tailwind v4 + shadcn/ui (Zinc base, light+dark) + recharts. NO hand-rolled CSS — everything lives in styles/tailwind.css (design tokens + a `.markdown` block + base/scrollbar rules). See the redesign spec/plan dated 2026-06-03.
    ...
    components/ui/          # vendored shadcn primitives (button/card/badge/dialog/dropdown-menu/table/switch/slider/toggle/toggle-group/tabs/input/label/command/checkbox/textarea) — generated by the CLI; don't hand-edit
    hooks/useDashboard.ts  # TanStack Query: NO refetchInterval (main is the sole poll driver) + onSnapshot push + cached seed; queryFn=api.refresh() so the manual Refresh button/⌘K can still force an on-demand poll
    hooks/useDigest.ts     # subscribes to main's 'digest' push (brief on-focus delta digest)
    hooks/{useHideActions,useReadState,useTriageVerdicts,useKeyboardNav,useSettings}.ts  # App orchestration extracted into focused hooks (hide/snooze, mark-read, lazy triage, j/k/⌘K nav, settings load)
    hooks/selection.ts     # moveSelection (pure) — j/k keyboard nav
    hooks/useNarrowViewport.ts  # matchMedia hook → true below NARROW_MAX_WIDTH=1024 (Tailwind `lg`). Drives the desktop↔narrow flip in lockstep: PrTable's table↔card switch AND App's grid↔tabbed layout. In jsdom (no matchMedia) it stays false → tests get the desktop layout.
    components/
      ...
      PrTable.tsx          # ONE configurable table (`columns: PrColumn[]`) for all three PR panels (needs-review/team/mine). emptyVariant + showAuthor props; renders pr-cells + RowActions per row. Below lg (useNarrowViewport) it renders a card layout instead of a table (kebab pinned top-right).
      ...
      Digest.tsx, DigestPane.tsx, ReviewPanel.tsx  # AI modals/pane (Dialog); markdown via react-markdown styled by the `.markdown` rules in tailwind.css. ReviewPanel renders diff context around each finding via DiffSnippet.
      DiffSnippet.tsx      # renders the diff hunk around a finding's anchor line (from ReviewResult.patch) — used by ReviewPanel; pure formatting in diff-snippet.ts
      ...
      # Pure, tested:
      pr-status.ts (mergeReadiness), sort-prs.ts, snooze.ts, activity-severity.ts, trend-metrics.ts, diff-snippet.ts   # pure, tested
```
...
- **`Settings.chartsCollapsed` is currently unwired (latent state).** The field is persisted and `useSettings` exposes an `onToggleCharts` setter, but nothing in the renderer consumes it — there is no trend-strip collapse button, so the value never moves off its default. Don't assume the trend strip can be collapsed today; treat `chartsCollapsed`/`onToggleCharts` as dead until a collapse control is wired up.
